 ...
 *
 * Both are checks rather than sources: one explains the boundary this regex needs by showing the
 * shapes it has to tell apart, and the other pins shadcn's vocabulary as data in order to refuse
 * it.
 *
 * Named rather than pattern-matched, and short on purpose — an exemption is a hole, and the files
 * here hold no CSS and no component read, so the hole has nothing in it that could break.
 */
