...
describe("RadioGroup (browser)", () => {
  it("fills the control of the chosen radio, leaving the dot as its centre", async () => {
    ...
    // `.rp-radio__indicator:empty::before` is what paints it, which is why the indicator has to
    // stay childless.
  ...
  describe("accessibility", () => {
    ...
    it("has no violations while invalid with a message", async () => {
      ...
      await nextTick();
      // `color-contrast` is scoped out for this one case: `[data-invalid] .rp-label` paints the
      // label in `--rp-danger`, which falls short of 4.5:1. Verified byte-identical on React at
