    ...
    template: `
      <div class="flex flex-col gap-8">
        <div v-for="size in sizes" :key="size" class="flex flex-col gap-2">
          <span class="text-sm font-medium text-[var(--rp-muted)] capitalize">{{ size }}</span>
    ...
    template: `
      <div class="flex flex-col gap-8">
        <div v-for="variant in variants" :key="variant" class="flex flex-col gap-2">
          <span class="text-sm font-medium text-[var(--rp-muted)] capitalize">{{ variant }}</span>
    ...
    template: `
      <div class="flex flex-col gap-8">
        <div v-for="layout in layouts" :key="layout" class="flex flex-col gap-2">
          <span class="text-sm font-medium text-[var(--rp-muted)] capitalize">{{ layout }}</span>
    ...
    template: `
      <div class="flex gap-16">
        <div v-for="variant in variants" :key="variant" class="flex flex-col gap-6">
          <span class="text-sm font-semibold text-[var(--rp-muted)] capitalize">{{ variant }}</span>
          <div v-for="size in sizes" :key="size" class="flex items-center gap-4">
            <span class="w-8 text-sm text-[var(--rp-muted)]">{{ size }}</span>
