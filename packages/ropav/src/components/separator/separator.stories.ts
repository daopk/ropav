...
export const WithContent: Story = {
  render: () => ({
    ...
    setup: () => ({ items }),
    template: `
      <div class="max-w-md space-y-4 rounded-[calc(var(--rp-radius)*3)] bg-[var(--rp-surface)] p-4 shadow-[var(--rp-surface-shadow)]">
        <div v-for="(item, index) in items" :key="item.title">
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 space-y-0">
              ...
              <p class="text-sm text-[var(--rp-muted)]">{{ item.subtitle }}</p>
            </div>
          </div>
