...
const props = withDefaults(defineProps<ColorInputGroupRootProps>(), {
  fullWidth: undefined,
...
// Each named apart from the prop it resolves: an identifier matching a prop name resolves to the
// prop inside the template, which would silently drop the value coming from the field. Unlike
// `InputGroup` the variant is never inherited — React's `ColorField` hands down no variant, so
// the group's own prop is the only source.
...
// a field holding more than one control keeps them apart.
...
// A group inside a field is presentational: the field is what assistive technology reads, so a
// second grouping around the control would only add noise. Standing alone it is a real group. A
// `role` from the caller wins either way, through attribute fallthrough.
