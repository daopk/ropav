  ...
  render: () => ({
    ...
    template: `
      <div class="flex w-full flex-col gap-4">
        ...
        <p class="w-full min-w-[300px] text-sm text-[var(--rp-muted)]">
    ...
    template: `
      <div class="flex flex-col gap-8">
        <div class="flex flex-col gap-2">
          <p class="text-sm font-medium text-[var(--rp-muted)]">HSB: Saturation vs Brightness (default)</p>
          <ColorArea default-value="hsl(30, 100%, 50%)">
            <ColorAreaThumb />
          </ColorArea>
        </div>
        <div class="flex flex-col gap-2">
          <p class="text-sm font-medium text-[var(--rp-muted)]">RGB: Red vs Green</p>
          <ColorArea default-value="rgb(255, 100, 50)" x-channel="red" y-channel="green">
            <ColorAreaThumb />
          </ColorArea>
        </div>
        <div class="flex flex-col gap-2">
          <p class="text-sm font-medium text-[var(--rp-muted)]">RGB: Blue vs Green</p>
      ...
      <div class="flex min-w-[300px] flex-col gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex flex-col gap-0.5">
            ...
            <span class="text-xs text-[var(--rp-muted)]">{{ color.toString("hex") }}</span>
          </div>
        </div>
      </div>
    `,
