    ...
    return {
      ...
      template: `
        <div class="w-full space-y-4">
          ...
          <div class="space-y-2 rounded-[calc(var(--rp-radius)*1.5)] border border-[var(--rp-border)] p-4">
            <p class="text-sm font-medium">Clear event info:</p>
            <div class="space-y-1 text-sm text-[var(--rp-muted)]">
              <p>Clear button clicked: {{ clearCount }} time(s)</p>
              <p v-if="selectedName" class="text-[var(--rp-success)]">
...
export const FullWidth: Story = {
  render: () => {
    ...
    return {
      ...
      template: `
        <Surface class="w-[380px] space-y-4 rounded-[calc(var(--rp-radius)*3)] p-6">
          <Autocomplete
            full-width
            :item-text-value="byName"
            :items="items"
    ...
    return {
      ...
      template: `
        <div class="space-y-4">
          ...
          <p class="text-sm text-[var(--rp-muted)]">Autocomplete is {{ isOpen ? "open" : "closed" }}</p>
        </div>
      `,
    };
  },
