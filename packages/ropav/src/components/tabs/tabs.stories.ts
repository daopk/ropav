      ...
      <div class="w-[380px]">
        ...
        >
          <TabsListContainer>
            <TabsList
              aria-label="Options"
              class="w-fit *:h-6 *:w-fit *:px-3 *:text-sm *:font-normal *:data-[selected=true]:text-[var(--rp-accent-foreground)]"
            >
              <TabsTab v-for="item in items" :id="item.id" :key="item.id">
                {{ item.label }}
                <TabsIndicator class="bg-[var(--rp-accent)]" />
              </TabsTab>
            </TabsList>
          </TabsListContainer>
          <TabsPanel v-for="item in items" :id="item.id" :key="item.id" class="pt-4">
        ...
        <div class="flex flex-col items-center">
          ...
          <div class="relative h-10 w-10">
            <p
              ...
              :key="level.id"
              :aria-hidden="selectedZoom !== level.id"
              class="absolute start-1/2 top-1/2 origin-center -translate-x-1/2 -translate-y-1/2 scale-75 text-[21px] font-medium text-[var(--rp-foreground)] opacity-0 transition-[scale,opacity] duration-[300ms] ease-[cubic-bezier(0.33,1,0.68,1)] data-[selected=true]:scale-100 data-[selected=true]:opacity-100 data-[selected=true]:delay-200"
          ...
          <footer class="mt-4 w-full px-4 text-center text-xs text-[var(--rp-muted)]/30 sm:text-sm">
