              ...
              <RadioControl>
                <RadioIndicator>
                  <template #default="{isSelected}">
                    <span v-if="isSelected" class="text-xs leading-none text-[var(--rp-background)]">✓</span>
                  </template>
                </RadioIndicator>
              </RadioControl>
              {{ plan }}
          ...
          <RadioGroup default-value="express" name="delivery">
            ...
            <div class="grid gap-x-4 md:grid-cols-3">
              <Radio v-for="option in deliveryOptions" :key="option.value" :value="option.value">
                <RadioContent
                  class="group relative flex w-full flex-col gap-6 rounded-[calc(var(--rp-radius)*1.5)] bg-[var(--rp-surface-tertiary)] px-5 py-4 transition-all data-[selected=true]:border-[var(--rp-accent)] data-[selected=true]:bg-[var(--rp-accent)]/10 data-[focus-visible=true]:bg-[var(--rp-accent)]/10"
                >
          ...
          <RadioGroup default-value="visa" name="payment">
            ...
            <div class="grid gap-x-4 md:grid-cols-2">
              <Radio v-for="option in paymentOptions" :key="option.value" :value="option.value">
                <RadioContent
                  class="group relative flex w-full flex-row items-start justify-start gap-4 rounded-[calc(var(--rp-radius)*1.5)] bg-[var(--rp-surface-tertiary)] px-5 py-4 transition-all data-[selected=true]:bg-[var(--rp-accent)]/10"
                >
                  <RadioControl class="absolute end-4 top-3 size-5">
                    <RadioIndicator />
                  </RadioControl>
                  <component :is="option.icon" class="size-6 text-[var(--rp-accent)]" />
                  <div class="flex flex-col gap-1">
                    <span>{{ option.title }}</span>
                    <Description>{{ option.description }}</Description>
                  </div>
