    ...
    template: `
      <div class="flex flex-col gap-6">
        <div class="flex flex-col gap-2">
          <h3 class="text-sm font-semibold text-[var(--rp-muted)]">Custom Border</h3>
          <div class="flex items-center gap-3">
            <ColorSwatch
        ...
        <div class="flex flex-col gap-2">
          <h3 class="text-sm font-semibold text-[var(--rp-muted)]">Custom Shadow</h3>
          <div class="flex items-center gap-3">
            <ColorSwatch
              v-for="color in colors"
              :key="color"
        ...
        <div class="flex flex-col gap-2">
          <h3 class="text-sm font-semibold text-[var(--rp-muted)]">Outline Style</h3>
          <div class="flex items-center gap-3">
            <ColorSwatch
              v-for="color in colors"
              :key="color"
    ...
    template: `
      <div class="flex flex-col gap-6">
        <div v-for="shape in shapes" :key="shape" class="flex flex-col gap-3">
          <h3 class="text-sm font-semibold text-[var(--rp-muted)] capitalize">{{ shape }}</h3>
          <div class="flex flex-col gap-3">
            <div v-for="size in sizes" :key="size" class="flex items-center gap-3">
              <div class="w-12 text-sm text-[var(--rp-muted)]">{{ size }}</div>
              <ColorSwatch
                v-for="color in colors"
                :key="color"
                :color="color"
