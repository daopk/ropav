    ...
    template: `
      <NumberField :default-value="99" :format-options="eur" :min-value="0" name="amount">
        ...
        <NumberFieldGroup class="flex">
          ...
          <div class="flex h-full flex-col border-s border-[var(--rp-field-placeholder,var(--rp-muted))]/15">
            <NumberFieldIncrementButton
  ...
  render: () => ({
    ...
    template: `
      <Form class="flex w-[280px] flex-col gap-4" @submit="onSubmit">
        ...
        <p v-if="submitted" class="text-sm text-[var(--rp-muted)]">Order submitted — {{ submitted }}</p>
      </Form>
    `,
  }),
};
