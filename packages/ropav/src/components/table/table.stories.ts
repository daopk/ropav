              ...
              <TableCell class="font-medium">
                <div class="flex items-center gap-2">#{{ user.id }}
                  <Button aria-label="Copy" is-icon-only size="sm" variant="ghost">
                    <IconCopy class="size-4 text-[var(--rp-muted)]" />
                  </Button>
                </div>
              </TableCell>
              <TableCell>
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex flex-col">
                    ...
                    <span class="text-xs text-[var(--rp-muted)]">{{ user.email }}</span>
                  </div>
                </div>
              </TableCell>
        ...
        <Table :variant="variant">
          <TableScrollContainer class="h-[280px] overflow-y-auto">
            <TableContent aria-label="Async loading" class="min-w-[600px]">
              <TableHeader class="sticky top-0 z-10 bg-[var(--rp-surface-secondary)]">
                <TableColumn
                  v-for="column of columns"
                  :id="column.id"
                  :key="column.id"
                    ...
                    <span class="flex items-center gap-1">
                      <TableExpandTrigger v-if="hasChildRows && isTreeColumn">
                        <Button aria-label="Expand row" is-icon-only size="sm" variant="ghost">
                          <IconChevronRight
                            class="size-4 text-[var(--rp-muted)] transition-transform duration-150"
            ...
            <TableContent aria-label="Empty state" class="h-full">
              ...
              <TableBody>
                <template #empty>
                  ...
                  >
                    <IconTray class="size-6 text-[var(--rp-muted)]" />
                    <span class="text-sm text-[var(--rp-muted)]">No results found</span>
                  </EmptyState>
                </template>
              </TableBody>
            </TableContent>
