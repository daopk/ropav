...
/**
 * Localized strings for colour components, copied from react-stately 3.49.0
 * (`packages/react-stately/intl/color/*.json`) and react-aria 3.51.0
 * (`packages/react-aria/intl/color/*.json`).
 *
 * Upstream keeps two tables because they belong to two packages; here there is one i18n
 * directory and no package boundary between them, and the key sets do not overlap, so they
 * are merged. The colour model reads the naming keys; the components read the rest.
 *
 * Channel names, lightness and chroma modifiers, and hue names share keys because `red`,
 * `green` and `blue` are each both a channel name and a hue name — the two readings differ
