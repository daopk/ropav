/**
 * The colour model's public type surface.
 *
 * Ported from React Stately's `color/types.ts` (react-stately 3.49.0). Kept in its own file
 * because it is the half consumers import: `Color` is an opaque interface with no public
 * constructor, so a caller reaching for a colour value goes through `parseColor` and then
...
/** A list of supported color spaces. */
...
/** The three channels of a color space, ordered for a two dimensional control. */
...
/**
 * Represents a color value. Every method that returns a `Color` returns a new one — a colour
 * is immutable, which is why `withChannelValue` exists instead of a setter.
 */
export interface Color {
  ...
  /** Returns the minimum, maximum, step and page step values for a given channel. */
  getChannelRange: (channel: ColorChannel) => ColorChannelRange;
  /**
   * Returns a localized color channel name for a given channel and locale, for use in visual
  ...
  /** Formats the numeric value for a given channel according to the provided locale. */
