/**
 * OKLCH colour generation for Ropav themes.
 *
...
/**
 * Drop the trailing zeros `toFixed` leaves behind, so `0.0000` reads as `0`.
 *
 * Cosmetic, but it has to happen here: `oxfmt` normalises number literals the same way, and
 * a generator whose output the formatter then rewrites would dirty the tree on every run.
 */
...
export function formatOklch(color: OklchColor): string {
  const l = trimZeros((color.l * 100).toFixed(2));
  const c = trimZeros(color.c.toFixed(4));
  const h = trimZeros(color.h.toFixed(2));
