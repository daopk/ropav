...
function getProviderDefaults(props: Readonly<ToastProviderProps>) {
    return withoutUndefined<Omit<ToastProps, 'open' | 'title' | 'description'>>({
        ...
        radius: props.radius,
        role: props.role,
        pauseOnHover: props.pauseOnHover ?? DEFAULT_TOAST_PAUSE_ON_HOVER,
        pauseOnFocus: props.pauseOnFocus ?? DEFAULT_TOAST_PAUSE_ON_FOCUS,
        closable: props.closable ?? DEFAULT_TOAST_CLOSABLE,
        closeLabel: props.closeLabel ?? DEFAULT_TOAST_CLOSE_LABEL,
    });
}
...
export function provideToast(props: Readonly<ToastProviderProps>): ToastProviderContext {
    const scopeId = useId();
    ...
    function create(type: ToastType, input: ToastInput, options?: ToastOptions) {
    ...
    function show(input: ToastInput, options?: ToastOptions) {
        return create('default', input, options);
    }

    function success(input: ToastInput, options?: ToastOptions) {
        return create('success', input, options);
    }

    function error(input: ToastInput, options?: ToastOptions) {
        return create('error', input, options);
    }

    function warning(input: ToastInput, options?: ToastOptions) {
        return create('warning', input, options);
    }

    function info(input: ToastInput, options?: ToastOptions) {
        return create('info', input, options);
    }

    function update(id: ToastId, options: ToastUpdateOptions) {
    ...
    function dismissInstance(instanceId: string, reason: ToastCloseReason = 'dismiss') {
        closeAt(
            items.value.findIndex((item) => item.instanceId === instanceId),
            reason,
        );
    }
