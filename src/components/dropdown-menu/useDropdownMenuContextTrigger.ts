...
export function useDropdownMenuContextTrigger(
    props: Readonly<DropdownMenuContextTriggerPrimitiveProps>,
) {
    ...
    function onPointerdown(event: PointerEvent) {
        if (isDisabled.value || (event.pointerType !== 'touch' && event.pointerType !== 'pen')) {
            return;
        }
    ...
    function onPointermove(event: PointerEvent) {
        ...
        if (distance > (props.longPressTolerance ?? 0)) clearLongPress();
    }
    ...
    return { isOpen: root.isOpen, rootAttrs, setElement };
}
